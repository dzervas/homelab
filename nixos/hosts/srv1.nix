...
in
{
  ...
  disko = {
    devices = {
      ...
      # Only used for SD image generation
      lvm_vg.mainpool.lvs.longhorn.size = lib.mkForce "8G";
    ...
    imageBuilder = {
      ...
      extraPostVM = ''${imagePkgs.zstd}/bin/zstd -T0 --rm "$out/root.raw"'';
    };
  };
  ...
  boot = {
    ...
    loader = {
      # Disable the shared EFI bootloader, not extlinux.
      grub.enable = false;
  ...
  system.build.installBootLoader = lib.mkForce (
    pkgs.writeShellScript "install-srv1-boot" ''
      set -eu
